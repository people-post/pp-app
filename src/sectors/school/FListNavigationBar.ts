...
export class FListNavigationBar extends Fragment {
  protected _fBtnPrev: Button;
  protected _fIdxInput: NumberInput;
  ...
  setIdx(idx: number): void { this._currentIdx = idx; }
  setNTotal(n: number): void { this._nTotal = n; }
  ...
  onInputChangeInNumberInputFragment(fInput: NumberInput, n: number): void {
    if (fInput.validate()) {
      this.#switchIdx(n - 1);
    }
  }
  ...
  #onNext(): void { this.#switchIdx(this._currentIdx + 1); }
