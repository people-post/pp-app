...
export class FQueueStatusMessage extends Fragment {
  ...
  updateStatus(n: number, nTotal: number): void {
    if (nTotal == 0) {
      ...
      this._fText.setClassName("s-font5 tw-text-red-600");
      this.#updateText(R.get("OUT_OF_SERVICE"));
    } else if (n == 0) {
      this._fText.setClassName("s-font5 tw-text-green-700");
      ...
      if (n < nTotal * 0.2) {
        ...
        this._fText.setClassName("s-font5 tw-text-yellow-600");
      } else {
        this._fText.setClassName("s-font5 tw-text-red-600");
      }
  ...
  clearStatus(): void { this.#updateText(""); }
