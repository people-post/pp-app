...
export class WcSession extends WindowController {
  ...
  topAction(type: string | symbol, ...args: unknown[]): void {
    switch (type) {
    case T_ACTION.LOGIN:
      this.#showLoginView(args[0] as View | undefined);
      break;
    case T_ACTION.SHOW_USER_INFO:
      this.#showUserInfoView(args[0] as string | null);
      break;
    case T_ACTION.SHOW_GROUP_INFO:
      this.#showUserGroupView(args[0] as string | null);
      break;
    case FwkT_ACTION.RELOAD_URL:
      this.#onReloadUrl();
      break;
    case FwkT_ACTION.PUSH_STATE:
      this._onPushState(args[0], args[1] as string);
      break;
    case FwkT_ACTION.REPLACE_STATE:
      this.#onReplaceState(args[0], args[1] as string);
      break;
    case FwkT_ACTION.SHOW_NOTICE:
      this.#showNotice(args[1] as string);
      break;
    case FwkT_ACTION.SHOW_DIALOG:
      this._showCustomDialog(args[1] as View, args[2] as string, args[3] as boolean | undefined);
      break;
    case FwkT_ACTION.CLOSE_DIALOG:
      this.#closeDialog();
      break;
    case FwkT_ACTION.SHOW_LAYER:
      this._pushLayer(args[1] as ViewLayer, args[2] as string);
      break;
    default:
      break;
    }
  ...
  _initLayer(lc: ViewLayer): void { lc.init(); }
  ...
  _pushView(view: View, title: string): void {
  ...
  _pushDialog(view: View, title: string): void { this.#pushDialog(view, title, true); }
  ...
  #onPopState(evt: PopStateEvent): void {
    if (evt.state) {
      if ((evt.state as StateData).obsolete) {
        history.back();
      } else {
  ...
  #onReplaceState(stateData: unknown, title: string): void {
    let state = this.#getState(stateData, title);
  ...
  #onReloadUrl(): void { location.reload(); }
  ...
  #showLoginView(nextView: View | undefined): void {
  ...
  #showUserGroupView(groupId: string | null): void {
    let v = new View();
  ...
  #pushDialog(view: View, title: string, enableCloseButton: boolean): void {
    if (!(this._childStack[0] instanceof LvDialog)) {
      let lc = new LvDialog();
