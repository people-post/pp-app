export class SearchIconOperator {
  press(iconElement: Element): void {
    ...
    if (!ee) return;
    let c = ee.firstElementChild as HTMLElement; // search-icon-circle
    let t = c.nextElementSibling as HTMLElement; // search-icon-top-bar
    let b = t.nextElementSibling as HTMLElement; // search-icon-bottom-bar
