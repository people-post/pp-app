...
const _CRC_RENDER_CONTROLLER = {
  TIP_LINK :
      `<a class="knowledge-tip" href="javascript:void(0)" data-pp-action="__ACTION_ID__" data-pp-args='["__TIP_MSG_ID__"]'>__TEXT__</a>`,
  FATAL_MSG :
...
export class RenderController extends Controller {
  ...
  _confirmDangerousOperation(msg: string, func: (() => void) | null): void {
    ...
    const ViewClass = Factory.getRequiredCtor<any>(T_OBJ.VIEW);
    const ConfirmActionClass =
        Factory.getRequiredCtor<any>(T_OBJ.CONFIRM_ACTION_FRAGMENT);
    let v = new ViewClass();
